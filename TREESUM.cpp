...
void GetLevelSum(map <int, vector <int> > & tree, map <int, vector <int> > & level_counts, int n, int k);
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < t; i++) {
		...
		GetLevelSum(tree, level_counts, n, k);
		if (i < t - 1) printf("\n");
		//answers.push_back(GetLevelSum(tree, level_counts, n, k));
	}

	/****for (int i = 0; i < t; i++) {
		//for (int j = 0; j < answers[i].size(); j++) printf("%lld\n", answers[i][j]);//cout << answers[i][j] << endl;
		//printf("\n");
	}****/
}
...
void GetLevelSum(map <int, vector <int> > & tree, map <int, vector <int> > & level_counts, int n, int k) {
	...
	for (int i = 0; i < n; i++) printf("%lld\n", (long long int)(sums[i]%1000000007));
